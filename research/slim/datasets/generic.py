...
from datasets.download_and_convert_generic import _get_filenames_and_classes
...
def get_split(split_name, dataset_dir, file_pattern=None, reader=None, perc_validation=_PERC_VALIDATION, labels_filename=_LABELS_FILENAME):
	...
	"""
	if perc_validation is None:
		perc_validation = _PERC_VALIDATION
	...
	num_validation = images_count*perc_validation//100
	...
	if dataset_utils.has_labels(dataset_dir + '/tfrecords', filename=labels_filename):
		labels_to_names = dataset_utils.read_label_file(dataset_dir + '/tfrecords', filename=labels_filename)
